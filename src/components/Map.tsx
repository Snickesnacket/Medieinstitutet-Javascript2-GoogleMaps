...
export const HomePage = () => {
  ...
  const handleOnSelect = async (item: Item) => {
    ...
    setSelected({ lat, lng });
    console.log(
      "selected variable containing lat and lang  in handleOnSelect",
      selected
    );
    setZoom(12);
    setSelectedOrt(item.name);
    console.log("selectedOrt area name in handleOnSelect", selectedOrt);
    setSearchParams({ query: item.name });
    navigate(`?query=${encodeURIComponent(item.name)}`);
    console.log(
      " navigate param is set the same as selectedOrt in handleOnSelect",
      navigate
    );
